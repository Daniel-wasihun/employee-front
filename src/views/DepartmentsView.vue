<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-extrabold text-white tracking-tight">Departments</h1>
        <p class="text-dark-400 mt-1">Manage company structure and teams</p>
      </div>
      ...
      <div v-for="dept in departments" :key="dept.id" class="card group hover:border-primary-500/30 transition-all duration-300">
        <div class="p-6">
          <div class="flex items-start justify-between mb-4">
            <div class="w-12 h-12 rounded-xl bg-dark-700 flex items-center justify-center text-primary-400 group-hover:bg-primary-600 group-hover:text-white transition-all">
            ...
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button @click="openEditModal(dept)" class="p-2 text-dark-400 hover:text-white transition-colors"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
              <button @click="confirmDelete(dept)" class="p-2 text-dark-400 hover:text-red-400 transition-colors"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
            </div>
          </div>
          
          <h3 class="text-xl font-bold text-white mb-2">{{ dept.name }}</h3>
          <p class="text-sm text-dark-400 line-clamp-2 mb-6 h-10">{{ dept.description || 'No description provided.' }}</p>
          
          <div class="flex items-center justify-between pt-4 border-t border-dark-700/50">
            <div class="flex flex-col">
              <span class="text-[10px] uppercase font-bold text-dark-500 tracking-wider">Manager</span>
              <span class="text-sm font-medium text-dark-200">{{ dept.managerName || 'Unassigned' }}</span>
            </div>
            <div class="bg-dark-700/50 px-3 py-1.5 rounded-xl border border-dark-600/50">
              <span class="text-sm font-bold text-white">{{ dept.employeeCount || 0 }}</span>
              <span class="text-[10px] text-dark-400 ml-1 font-bold uppercase tracking-widest">Staff</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <BaseModal v-model="showModal" :title="isEditing ? 'Edit Department' : 'Create Department'">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label class="block text-xs font-bold text-dark-300 mb-1.5 uppercase">Department Name</label>
          <input v-model="form.name" type="text" required class="input" placeholder="e.g. Engineering">
        </div>
        <div>
          <label class="block text-xs font-bold text-dark-300 mb-1.5 uppercase">Description</label>
          <textarea v-model="form.description" class="input h-24 py-3" placeholder="Tell us about this department..."></textarea>
        </div>
