<template>
  <div class="space-y-8 animate-in fade-in duration-500">
    <!-- Header -->
    <div>
      <h1 class="text-3xl font-extrabold text-[var(--text-main)] tracking-tight">Dashboard Overview</h1>
      <p class="text-[var(--text-muted)] mt-1">Real-time workforce analytics and metrics</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="stat in statsCards" :key="stat.title" class="card p-6 flex items-center gap-5 group hover:border-primary-500/30 transition-all duration-300">
        ...
        <div>
          <p class="text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-wider mb-1">{{ stat.title }}</p>
          <p class="text-2xl font-bold text-[var(--text-main)] tabular-nums">
            <template v-if="loading"><div class="h-8 w-16 bg-[var(--border-subtle)] animate-pulse rounded" /></template>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 card overflow-hidden">
        <div class="px-6 py-4 border-b border-[var(--border-subtle)] flex items-center justify-between bg-[var(--bg-input)]/30">
          ...
          <router-link to="/employees" class="text-xs font-semibold text-primary-600 hover:text-primary-500 transition-colors uppercase tracking-widest">View All</router-link>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead>
              <tr class="bg-[var(--bg-input)]/50 text-left">
                <th class="px-6 py-3 text-xs font-bold text-[var(--text-dim)] uppercase tracking-wider">Employee</th>
                <th class="px-6 py-3 text-xs font-bold text-[var(--text-dim)] uppercase tracking-wider">Position</th>
                <th class="px-6 py-3 text-xs font-bold text-[var(--text-dim)] uppercase tracking-wider">Department</th>
                <th class="px-6 py-3 text-xs font-bold text-[var(--text-dim)] uppercase tracking-wider">Status</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-[var(--border-subtle)]">
              <tr v-for="emp in stats?.recentHires" :key="emp.id" class="hover:bg-primary-500/5 transition-colors">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-[var(--bg-input)] flex items-center justify-center text-xs font-bold text-[var(--text-main)] border border-[var(--border-subtle)] shadow-sm">
                      {{ emp.firstName.charAt(0) }}{{ emp.lastName.charAt(0) }}
                    </div>
                    <span class="text-sm font-medium text-[var(--text-main)]">{{ emp.firstName }} {{ emp.lastName }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-sm text-[var(--text-muted)]">{{ emp.position }}</td>
                <td class="px-6 py-4 text-sm text-[var(--text-muted)]">{{ emp.departmentName }}</td>
                <td class="px-6 py-4">
      ...
      <div class="card p-6 flex flex-col h-full">
        <h3 class="font-bold text-[var(--text-main)] flex items-center gap-2 mb-6">
        ...
        <div class="flex-1 space-y-5">
          <div v-for="(count, dept) in stats?.byDepartment" :key="dept" class="space-y-1.5">
            <div class="flex justify-between text-xs font-medium">
              <span class="text-[var(--text-main)]">{{ dept }}</span>
              <span class="text-[var(--text-dim)]">{{ count }} employees</span>
            </div>
            <div class="h-2 w-full bg-[var(--bg-input)] rounded-full overflow-hidden border border-[var(--border-subtle)]">
              <div 
                class="h-full bg-gradient-to-r from-primary-400 to-primary-600 rounded-full transition-all duration-1000 shadow-sm"
