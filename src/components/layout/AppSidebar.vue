<template>
  <aside 
    class="fixed inset-y-0 left-0 z-40 w-64 bg-dark-850 border-r border-dark-700/50 transition-transform duration-300 transform md:relative md:translate-x-0"
    :class="isOpen ? 'translate-x-0' : '-translate-x-full'"
  >
    <!-- Logo -->
    <div class="h-16 flex items-center gap-3 px-6 border-b border-dark-700/50">
      <div class="w-8 h-8 rounded-xl bg-primary-600 flex items-center justify-center shadow-lg shadow-primary-900/40">
      ...
      <h1 class="text-xl font-bold text-white tracking-tight">EMS<span class="text-primary-500">Pro</span></h1>
    </div>

    <!-- Navigation -->
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
      <p class="px-2 mb-4 text-[10px] font-bold text-dark-500 uppercase tracking-[2px]">Main Menu</p>
      ...
      <router-link 
        ...
        :class="$route.path === item.path ? 'bg-primary-600 text-white shadow-lg shadow-primary-900/20' : 'text-dark-300 hover:text-white hover:bg-dark-700'"
      >
      ...
      <div v-if="adminItems.length > 0" class="pt-6">
        <p class="px-2 mb-4 text-[10px] font-bold text-dark-500 uppercase tracking-[2px]">Administration</p>
        <router-link 
    ...
    <div class="p-6 border-t border-dark-700/50">
      <div class="bg-dark-800 rounded-2xl p-4 flex items-center gap-3">
        <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
        <span class="text-xs text-dark-300 font-medium">System Online</span>
      </div>
    </div>
  </aside>
...
// Icons as functional components to avoid runtime-compiler requirement
